sap.ui.define([
	...
	function extendMetadataUrlParameters(aUrlParametersToAdd, oMetadataUrlParams, sServiceUrl) {
		...
		aUrlParametersToAdd.forEach(function(sUrlParam) {
			...
			if (sUrlParam === "sap-language") {
				sLanguage = "DE"; //sap.ushell.Container.getUser().getLanguage();
	...
	return {
		...
		createODataModel: function(oOptions) {
			...
			if (!oOptions.url) {
				jQuery.sap.log.error("Please provide a url when you want to create an ODataModel", "com/pr36/pic.model.models.createODataModel");
				return null;
			}
